...
''');
    ...
    for (final field in fields) {
      ...
      if (field.isOptional) {
        buffer.writeln("      if (obj.${field.name} != null) '$jsonKey': _serializeValue(obj.${field.name}),");
      } else {
    ...
    buffer.writeln('    final serialized = JsonMapper.serialize(this);');
    buffer.writeln('    return serialized is Map<String, dynamic> ? serialized as Map<String, dynamic> : <String, dynamic>{};');
    buffer.writeln('  }');
    buffer.writeln('}');
    buffer.writeln();
    ...
    buffer.writeln('    return JsonMapper.deserialize<$className>(jsonString);');
    buffer.writeln('  }');
    ...
    buffer.writeln('  static List<$className> listFromJsonString(String jsonString) {');
    buffer.writeln('    try {');
    buffer.writeln('      return JsonMapper.deserialize<List<$className>>(jsonString) ?? [];');
    buffer.writeln('    } catch (e) {');
    buffer.writeln('      return [];');
    buffer.writeln('    }');
    buffer.writeln('  }');
    buffer.writeln('  /// 从 List 创建对象列表');
    buffer.writeln('  static List<$className> listFromMaps(List<Map<String, dynamic>> maps) {');
    buffer.writeln('    return maps.map((map) => fromMap(map)).whereType<$className>().toList();');
    buffer.writeln('  }');
    buffer.writeln('}');
...
class _FieldInfo {
  ...
  _FieldInfo({required this.name, required this.type, required this.isOptional});
}
